...
class HousekeepingSelectors {
  ...
  static List<Cleaner> getFilteredCleaners(AppState state) {
    ...
    return cleaners.where((cleaner) {
      ...
      if (filters.isAvailable != null &&
          cleaner.isAvailable != filters.isAvailable) {
        return false;
      }

      // Filter by specialties
      if (filters.specialties.isNotEmpty) {
        final hasSpecialty = filters.specialties.any(
          (specialty) => cleaner.specialties.contains(specialty),
        );
        if (!hasSpecialty) {
          return false;
        }
  ...
  static List<Cleaner> getAvailableCleaners(AppState state) {
    return getCleaners(state)
        .where(
          (cleaner) =>
              cleaner.isAvailable && cleaner.status == CleanerStatus.active,
        )
        .toList();
  }
  ...
  static List<Cleaner> getActiveCleanersOnly(AppState state) {
    return getCleaners(
      state,
    ).where((cleaner) => cleaner.status == CleanerStatus.active).toList();
  }
  ...
  static Customer? getCustomerById(AppState state, String id) {
    return getCustomers(
      state,
    ).where((customer) => customer.id == id).firstOrNull;
  }
  ...
  static List<HousekeepingService> getServicesByCategory(
    AppState state,
    ServiceCategory category,
  ) {
    return getServices(
      state,
    ).where((service) => service.category == category).toList();
  }
  ...
  static List<Booking> getBookingsForCustomer(
    AppState state,
    String customerId,
  ) {
    return getBookings(
  ...
  static List<Booking> getBookingsForService(AppState state, String serviceId) {
    return getBookings(
      state,
    ).where((booking) => booking.serviceId == serviceId).toList();
  }
    ...
    return getBookings(state)
        .where(
          (booking) =>
              ...
              booking.scheduledDate.isBefore(endOfDay),
        )
        .toList();
  }
    ...
    return getBookings(state)
        .where(
          (booking) =>
              ...
              booking.scheduledDate.isBefore(endOfWeek),
        )
        .toList();
  }
