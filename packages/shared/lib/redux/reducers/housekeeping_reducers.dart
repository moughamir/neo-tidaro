...
final Reducer<HousekeepingState>
housekeepingReducer = combineReducers<HousekeepingState>([
  // Loading states
  TypedReducer<HousekeepingState, SetHousekeepingLoadingAction>(
    _setLoadingReducer,
  ),
  TypedReducer<HousekeepingState, SetHousekeepingRefreshingAction>(
    _setRefreshingReducer,
  ),

  // Error handling
  TypedReducer<HousekeepingState, SetHousekeepingErrorAction>(_setErrorReducer),
  TypedReducer<HousekeepingState, ClearHousekeepingErrorAction>(
    _clearErrorReducer,
  ),
  TypedReducer<HousekeepingState, SetHousekeepingLastUpdatedAction>(
    _setLastUpdatedReducer,
  ),

  // Metrics
  TypedReducer<HousekeepingState, SetHousekeepingMetricsAction>(
    _setMetricsReducer,
  ),
  ...
  TypedReducer<HousekeepingState, UpdateBookingStatusAction>(
    _updateBookingStatusReducer,
  ),
  TypedReducer<HousekeepingState, AssignCleanerToBookingAction>(
    _assignCleanerToBookingReducer,
  ),
  TypedReducer<HousekeepingState, CancelBookingAction>(_cancelBookingReducer),
  TypedReducer<HousekeepingState, UpdateBookingFiltersAction>(
    _updateBookingFiltersReducer,
  ),
  ...
  TypedReducer<HousekeepingState, UpdateCleanerStatusAction>(
    _updateCleanerStatusReducer,
  ),
  TypedReducer<HousekeepingState, UpdateCleanerAvailabilityAction>(
    _updateCleanerAvailabilityReducer,
  ),
  TypedReducer<HousekeepingState, UpdateCleanerFiltersAction>(
    _updateCleanerFiltersReducer,
  ),
  ...
  TypedReducer<HousekeepingState, UpdateCustomerFiltersAction>(
    _updateCustomerFiltersReducer,
  ),
  ...
  TypedReducer<HousekeepingState, ToggleServiceStatusAction>(
    _toggleServiceStatusReducer,
  ),
]);
...
HousekeepingState _setLoadingReducer(
  HousekeepingState state,
  SetHousekeepingLoadingAction action,
) {
  return state.copyWith(isLoading: action.isLoading);
}

HousekeepingState _setRefreshingReducer(
  HousekeepingState state,
  SetHousekeepingRefreshingAction action,
) {
  return state.copyWith(isRefreshing: action.isRefreshing);
}

HousekeepingState _setErrorReducer(
  HousekeepingState state,
  SetHousekeepingErrorAction action,
) {
  return state.copyWith(error: action.error);
}

HousekeepingState _clearErrorReducer(
  HousekeepingState state,
  ClearHousekeepingErrorAction action,
) {
  return state.copyWith(error: null);
}

HousekeepingState _setLastUpdatedReducer(
  HousekeepingState state,
  SetHousekeepingLastUpdatedAction action,
) {
...
HousekeepingState _setMetricsReducer(
  HousekeepingState state,
  SetHousekeepingMetricsAction action,
) {
  return state.copyWith(metrics: action.metrics, lastUpdated: DateTime.now());
}
...
HousekeepingState _setBookingsReducer(
  HousekeepingState state,
  SetBookingsAction action,
) {
  return state.copyWith(bookings: action.bookings);
}

HousekeepingState _selectBookingReducer(
  HousekeepingState state,
  SelectBookingAction action,
) {
  return state.copyWith(selectedBooking: action.booking);
}

HousekeepingState _createBookingReducer(
  HousekeepingState state,
...
HousekeepingState _updateBookingReducer(
  HousekeepingState state,
  ...
  return state.copyWith(
    ...
    selectedBooking: state.selectedBooking?.id == action.booking.id
        ? action.booking
        : state.selectedBooking,
  );
}

HousekeepingState _updateBookingStatusReducer(
  HousekeepingState state,
...
HousekeepingState _assignCleanerToBookingReducer(
  HousekeepingState state,
  AssignCleanerToBookingAction action,
) {
  final updatedBookings = state.bookings.map((booking) {
    if (booking.id == action.bookingId) {
...
HousekeepingState _cancelBookingReducer(
  HousekeepingState state,
...
HousekeepingState _updateBookingFiltersReducer(
  HousekeepingState state,
  UpdateBookingFiltersAction action,
) {
...
HousekeepingState _setCleanersReducer(
  HousekeepingState state,
  SetCleanersAction action,
) {
  return state.copyWith(cleaners: action.cleaners);
}

HousekeepingState _selectCleanerReducer(
  HousekeepingState state,
  SelectCleanerAction action,
) {
  return state.copyWith(selectedCleaner: action.cleaner);
}

HousekeepingState _createCleanerReducer(
  HousekeepingState state,
  CreateCleanerAction action,
) {
  final updatedCleaners = List<Cleaner>.from(state.cleaners)
    ..add(action.cleaner);
  return state.copyWith(cleaners: updatedCleaners);
}

HousekeepingState _updateCleanerReducer(
  HousekeepingState state,
  ...
  return state.copyWith(
    ...
    selectedCleaner: state.selectedCleaner?.id == action.cleaner.id
        ? action.cleaner
        : state.selectedCleaner,
  );
}

HousekeepingState _updateCleanerStatusReducer(
  HousekeepingState state,
...
HousekeepingState _updateCleanerAvailabilityReducer(
  HousekeepingState state,
  UpdateCleanerAvailabilityAction action,
) {
  final updatedCleaners = state.cleaners.map((cleaner) {
    if (cleaner.id == action.cleanerId) {
...
HousekeepingState _updateCleanerFiltersReducer(
  HousekeepingState state,
  UpdateCleanerFiltersAction action,
) {
...
HousekeepingState _setCustomersReducer(
  HousekeepingState state,
  SetCustomersAction action,
) {
  return state.copyWith(customers: action.customers);
}

HousekeepingState _selectCustomerReducer(
  HousekeepingState state,
  SelectCustomerAction action,
) {
  return state.copyWith(selectedCustomer: action.customer);
}

HousekeepingState _createCustomerReducer(
  HousekeepingState state,
...
HousekeepingState _updateCustomerReducer(
  HousekeepingState state,
  ...
  return state.copyWith(
    ...
    selectedCustomer: state.selectedCustomer?.id == action.customer.id
        ? action.customer
        : state.selectedCustomer,
  );
}

HousekeepingState _updateCustomerFiltersReducer(
  HousekeepingState state,
  UpdateCustomerFiltersAction action,
) {
...
HousekeepingState _setServicesReducer(
  HousekeepingState state,
...
HousekeepingState _selectServiceReducer(
  HousekeepingState state,
  SelectServiceAction action,
) {
  return state.copyWith(selectedService: action.service);
}

HousekeepingState _createServiceReducer(
  HousekeepingState state,
  CreateServiceAction action,
) {
  final updatedServices = List<HousekeepingService>.from(state.services)
    ..add(action.service);
  return state.copyWith(services: updatedServices);
}

HousekeepingState _updateServiceReducer(
  HousekeepingState state,
  ...
  return state.copyWith(
    ...
    selectedService: state.selectedService?.id == action.service.id
        ? action.service
        : state.selectedService,
  );
}

HousekeepingState _toggleServiceStatusReducer(
  HousekeepingState state,
  ToggleServiceStatusAction action,
) {
  final updatedServices = state.services.map((service) {
  ...
  final updatedSelectedService = state.selectedService?.id == action.serviceId
      ? state.selectedService!.copyWith(
